...
public class Brand extends JFrame implements ActionListener,MouseListener
{
...
//		panel1.setBorder(new LineBorder(new Color(0, 0, 0)));
		...
		panel3.setBorder(new TitledBorder(null, "STATUS", TitledBorder.LEADING, TitledBorder.TOP, null, SystemColor.textHighlight));
...
//		scrollPane.setViewportView(table_1);
		...
		scrollPane.setBounds(1030, 240, 530, 600);
		getContentPane().add(scrollPane);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getSource()==add)
		{
			...
			String brandString=comboBox1.getSelectedItem().toString();
			
			
			DbConnection co=new DbConnection();
			try 
			{
				PreparedStatement pst=co.c.prepareStatement("INSERT INTO brand (brand,status) VALUES (?,?)");
				pst.setString(1, statusString);
				pst.setString(2, brandString);
				pst.executeUpdate();
		...
		if(e.getSource()==update)
		{
			...
			String brandString=textField1.getText();
			String statusString=comboBox1.getSelectedItem().toString();
			try 
			{
				DbConnection co=new DbConnection();
				PreparedStatement pst=co.c.prepareStatement("UPDATE brand SET brand=?,status=? WHERE id=?");
				pst.setString(1, brandString);
				pst.setString(2, statusString);
				pst.setInt(3, id);
				pst.executeUpdate();
				JOptionPane.showMessageDialog(null, "Brand Is Updated");
				textField1.setText("");
				comboBox1.setSelectedIndex(-1);
				textField1.requestFocus();
				table_update();
		...
		if(e.getSource()==delete)
		{
			...
			if(diallogresult==JOptionPane.YES_OPTION)
			{
				...
				try {
					PreparedStatement pst=co.c.prepareStatement("DELETE FROM brand WHERE id=?");
					pst.setInt(1, id);
					pst.executeUpdate();
					JOptionPane.showMessageDialog(null, "Brand Is Deleted");
					textField1.setText("");
					comboBox1.setSelectedIndex(-1);
					textField1.requestFocus();
					table_update();
